-- =========================================================
-- A股情绪周期复盘与历史相似行情回测系统
-- ClickHouse 完整初始化脚本 V2
-- 文件名：init_ck.sql
-- 生成日期：2026-06-30
-- 适用版本：ClickHouse 23+
--
-- 这版已经按最终交付包 Java 代码中的 *Sql.java / *Mapper.java 重新扫描对齐。
-- 覆盖 39 张 ClickHouse 表：
--   输入快照、Engine输出快照、页面查询快照、回测、Agent审计等。
--
-- 执行方式：
--   clickhouse-client --multiquery < init_ck.sql
-- =========================================================
...
-- =========================================================
-- 最终代码扫描后补齐的 ClickHouse 支撑表
-- 补齐表数量：9
-- =========================================================

create table if not exists astock_analysis.historical_cycle_sample_factor
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
    rule_version_id UInt64 default 0 comment '规则版本ID',
    sample_id UInt64 default 0 comment '样本ID',
    report_id UInt64 default 0 comment '报告ID',
    stock_code String default '' comment '股票代码',
    stock_name String default '' comment '股票名称',
    sector_code String default '' comment '板块编码',
    sector_name String default '' comment '板块名称',
    ...
    pattern_code String default '' comment '模式编码',
    pattern_name String default '' comment '模式名称',
    risk_code String default '' comment '风险编码',
    risk_name String default '' comment '风险名称',
    stage_code LowCardinality(String) default '' comment '阶段编码',
    leader_type LowCardinality(String) default '' comment '龙头类型',
    match_type LowCardinality(String) default '' comment '匹配类型',
    status LowCardinality(String) default '' comment '状态',
    rank_no UInt32 default 0 comment '排名',
    score Decimal(18,6) default 0 comment '通用评分',
    strength_score Decimal(18,6) default 0 comment '强度分',
    similarity_score Decimal(18,6) default 0 comment '相似度',
    risk_score Decimal(18,6) default 0 comment '风险分',
    win_rate Decimal(18,6) default 0 comment '胜率',
    avg_return Decimal(18,6) default 0 comment '平均收益',
    avg_drawdown Decimal(18,6) default 0 comment '平均回撤',
    future_1d_return Decimal(18,6) default 0 comment '未来1日收益，仅历史/回测使用',
    future_3d_return Decimal(18,6) default 0 comment '未来3日收益，仅历史/回测使用',
    future_5d_return Decimal(18,6) default 0 comment '未来5日收益，仅历史/回测使用',
    future_10d_return Decimal(18,6) default 0 comment '未来10日收益，仅历史/回测使用',
    max_drawdown Decimal(18,6) default 0 comment '最大回撤，仅历史/回测使用',
    evidence_json String default '{}' comment '证据JSON',
    risk_json String default '{}' comment '风险JSON',
    features String default '{}' comment '扩展字段JSON',
    created_at DateTime default now() comment '创建时间'
...
engine = MergeTree
partition by toYYYYMM(trade_date)
order by (trade_date, market_scope, task_id, stock_code, sample_id)
comment '历史周期样本因子明细表';

create table if not exists astock_analysis.historical_following_performance
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
...
create table if not exists astock_analysis.leader_similarity_match
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
...
create table if not exists astock_analysis.mainline_similarity_match
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
...
create table if not exists astock_analysis.pattern_backtest_result
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
...
create table if not exists astock_analysis.risk_similarity_match
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
...
create table if not exists astock_analysis.sector_daily_snapshot
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
...
create table if not exists astock_analysis.sector_stock_mapping_snapshot
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
...
create table if not exists astock_analysis.trend_leader_snapshot
(
    trade_date Date comment '交易日',
    market_scope LowCardinality(String) default 'A_SHARE' comment '市场范围',
    task_id UInt64 default 0 comment '任务ID',
