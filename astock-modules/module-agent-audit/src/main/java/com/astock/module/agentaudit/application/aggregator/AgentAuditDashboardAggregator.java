package com.astock.module.agentaudit.application.aggregator;

import com.astock.common.annotation.NoMockAllowed;
import com.astock.common.data.PageDataQualityVO;
import com.astock.common.data.PageSnapshotBundle;
import com.astock.common.data.RequiredSnapshot;
import com.astock.infrastructure.dataquality.DataQualityQueryService;
import com.astock.module.agentaudit.api.vo.AgentAuditDashboardVO;
import com.astock.module.agentaudit.application.query.AgentAuditDashboardQuery;
import com.astock.module.agentaudit.domain.repository.AgentAuditPageRepository;
import com.astock.module.agentaudit.infrastructure.converter.AgentAuditConverter;
import org.springframework.stereotype.Component;

import java.time.LocalDate;
import java.util.List;

@Component
@NoMockAllowed
public class AgentAuditDashboardAggregator {
    private final DataQualityQueryService dataQualityQueryService;
    private final AgentAuditPageRepository pageRepository;
    private final AgentAuditConverter converter;

    public AgentAuditDashboardAggregator(DataQualityQueryService dataQualityQueryService,
                       AgentAuditPageRepository pageRepository,
                       AgentAuditConverter converter) {
        this.dataQualityQueryService = dataQualityQueryService;
        this.pageRepository = pageRepository;
        this.converter = converter;
    }

    public AgentAuditDashboardVO aggregate(AgentAuditDashboardQuery query) {
        LocalDate tradeDate = query.getTradeDate() == null ? LocalDate.now() : query.getTradeDate();
        String marketScope = query.getMarketScope() == null ? "A_SHARE" : query.getMarketScope();
        query.setTradeDate(tradeDate);
        query.setMarketScope(marketScope);

        PageDataQualityVO quality = dataQualityQueryService.checkPage(
                "PAGE_15_AGENT_AUDIT",
                tradeDate,
                marketScope,
                List.of(new RequiredSnapshot("PAGE_15_AGENT_AUDIT", "agent_audit_code_scan_detail", true))
        );

        if (!Boolean.TRUE.equals(quality.getDataComplete())) {
            return converter.convert(query, quality, new PageSnapshotBundle());
        }

        PageSnapshotBundle bundle = pageRepository.queryPage(tradeDate, marketScope, 50);
        return converter.convert(query, quality, bundle);
    }
}
